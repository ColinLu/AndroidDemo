...
#define PACKAGENAME "com/colin/library/readpdf"
...
static globals *get_globals(JNIEnv *env, jobject thiz) {
	globals *glo = (globals *) (void *) ((*env)->GetLongField(env, thiz,
			global_fid));
	if (glo != NULL) {
		glo->env = env;
		glo->thiz = thiz;
...
JNI_FN(MuPDFCore_openFile) (JNIEnv * env, jobject thiz, jstring jfilename) {
	...
	fz_try(ctx)
	{
		...
		fz_catch(ctx)
		{
			fz_throw(ctx, FZ_ERROR_GENERIC, "Cannot open document: '%s'",
					filename);
		}
		LOGE("Done!");
	}
...
static int bufferStreamRead(fz_stream *stream, unsigned char *buf, int len) {
	...
	JNIEnv *env = glo->env;
	jbyteArray array = (jbyteArray) (void *) ((*env)->GetObjectField(env,
			glo->thiz, buffer_fid));
...
static void update_changed_rects(globals *glo, page_cache *pc,
		pdf_document *idoc) {
	fz_annot *annot;

	pdf_update_page(idoc, (pdf_page *) pc->page);
	while ((annot = (fz_annot *) pdf_poll_changed_annot(idoc,
			(pdf_page *) pc->page)) != NULL) {
...
JNI_FN(MuPDFCore_drawPage) (JNIEnv *env, jobject thiz, jobject bitmap,
	...
	LOGE("Rendering page(%d)=%dx%d patch=[%d,%d,%d,%d]", pc->number, pageW,
			pageH, patchX, patchY, patchW, patchH);
	...
	fz_try(ctx)
	{
		...
		if (idoc) {
			...
			drop_changed_rects(ctx,
					hq ? &pc->hq_changed_rects : &pc->changed_rects);
		}
		...
		if (pc->annot_list == NULL) {
			...
			for (annot = fz_first_annot(doc, pc->page); annot; annot =
					fz_next_annot(doc, annot))
		...
		pix = fz_new_pixmap_with_bbox_and_data(ctx, glo->colorspace, &bbox,
				pixels);
...
JNIEXPORT jboolean JNICALL
JNI_FN(MuPDFCore_updatePageInternal) (JNIEnv *env, jobject thiz,
		jobject bitmap, int page, int pageW, int pageH, int patchX, int patchY,
	...
	if (pc == NULL) {
		...
		return JNI_FN(MuPDFCore_drawPage)(env, thiz, bitmap, pageW, pageH,
	...
	fz_try(ctx)
	{
		...
		for (crect = hq ? pc->hq_changed_rects : pc->changed_rects; crect;
				crect = crect->next) {
			fz_irect abox;
			...
			LOGI("Update rectangle (%d, %d, %d, %d)", abox.x0, abox.y0, abox.x1,
					abox.y1);
			if (!fz_is_empty_irect(&abox)) {
				...
				if (pc->annot_list)
					fz_run_display_list(pc->annot_list, dev, &ctm, &arect,
							NULL);
				fz_free_device(dev);
				dev = NULL;
			}
...
static int countOutlineItems(fz_outline *outline) {
	int count = 0;

	while (outline) {
		if (outline->dest.kind == FZ_LINK_GOTO
				&& outline->dest.ld.gotor.page >= 0 && outline->title)
			count++;
...
static int fillInOutlineItems(JNIEnv * env, jclass olClass, jmethodID ctor,
		jobjectArray arr, int pos, fz_outline *outline, int level) {
	while (outline) {
		if (outline->dest.kind == FZ_LINK_GOTO) {
...
JNIEXPORT jboolean JNICALL
JNI_FN(MuPDFCore_authenticatePasswordInternal) (JNIEnv *env, jobject thiz,
		jstring password) {
	const char *pw;
	int result;
	globals *glo = get_globals(env, thiz);
...
JNI_FN(MuPDFCore_getOutlineInternal) (JNIEnv * env, jobject thiz) {
	...
	if (olClass == NULL)
		return NULL;
	ctor = (*env)->GetMethodID(env, olClass, "<init>",
	...
	arr = (*env)->NewObjectArray(env, nItems, olClass, NULL);
	if (arr == NULL)
		return NULL;
...
JNI_FN(MuPDFCore_searchPage) (JNIEnv * env, jobject thiz, jstring jtext) {
	...
	fz_try(ctx)
	{
		if (glo->hit_bbox == NULL)
			glo->hit_bbox = fz_malloc_array(ctx, MAX_SEARCH_HITS,
					sizeof(*glo->hit_bbox));
		...
		hit_count = fz_search_text_page(ctx, text, str, glo->hit_bbox,
		MAX_SEARCH_HITS);
	}
	fz_always(ctx)
	{
	...
	for (i = 0; i < hit_count; i++) {
		rect = (*env)->NewObject(env, rectClass, ctor,
				(float) (glo->hit_bbox[i].x0), (float) (glo->hit_bbox[i].y0),
				(float) (glo->hit_bbox[i].x1), (float) (glo->hit_bbox[i].y1));
		if (rect == NULL)
			return NULL;
...
JNI_FN(MuPDFCore_text) (JNIEnv * env, jobject thiz) {
	...
	fz_try(ctx)
	{
		...
		if (barr == NULL)
			fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");
		...
		for (b = 0; b < text->len; b++) {
			...
			for (l = 0; l < block->len; l++) {
				...
				for (s = 0, span = line->first_span; span;
						s++, span = span->next) {
					jobjectArray *carr = (*env)->NewObjectArray(env, span->len,
							textCharClass, NULL);
					if (carr == NULL)
					...
					for (c = 0; c < span->len; c++) {
						...
						jobject cobj = (*env)->NewObject(env, textCharClass,
								ctor, bbox.x0, bbox.y0, bbox.x1, bbox.y1,
								ch->c);
						if (cobj == NULL)
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addMarkupAnnotationInternal) (JNIEnv * env, jobject thiz,
		jobjectArray points, fz_annot_type type) {
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	pdf_document *idoc = pdf_specifics(doc);
	...
	fz_try(ctx)
	{
		...
		annot = (fz_annot *) pdf_create_annot(idoc, (pdf_page *) pc->page,
				type);

		pdf_set_markup_annot_quadpoints(idoc, (pdf_annot *) annot, pts, n);
		pdf_set_markup_appearance(idoc, (pdf_annot *) annot, color, alpha,
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal) (JNIEnv * env, jobject thiz,
		jobjectArray arcs) {
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	fz_try(ctx)
	{
		...
		for (i = 0; i < n; i++) {
			jobjectArray arc = (jobjectArray)(*env)->GetObjectArrayElement(env,
					arcs, i);
		...
		pdf_set_ink_annot_list(idoc, (pdf_annot *) annot, pts, counts, n, color,
		INK_THICKNESS);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_deleteAnnotationInternal) (JNIEnv * env, jobject thiz,
		int annot_index) {
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_getPageLinksInternal) (JNIEnv * env, jobject thiz,
		int pageNumber) {
	...
	if (linkInfoClass == NULL)
		return NULL;
	linkInfoInternalClass = (*env)->FindClass(env,
	PACKAGENAME "/LinkInfoInternal");
	if (linkInfoInternalClass == NULL)
		return NULL;
	linkInfoExternalClass = (*env)->FindClass(env,
	PACKAGENAME "/LinkInfoExternal");
	if (linkInfoExternalClass == NULL)
		return NULL;
	linkInfoRemoteClass = (*env)->FindClass(env, PACKAGENAME "/LinkInfoRemote");
	if (linkInfoRemoteClass == NULL)
		return NULL;
	ctorInternal = (*env)->GetMethodID(env, linkInfoInternalClass, "<init>",
			"(FFFFI)V");
	if (ctorInternal == NULL)
		return NULL;
	ctorExternal = (*env)->GetMethodID(env, linkInfoExternalClass, "<init>",
			"(FFFFLjava/lang/String;)V");
	if (ctorExternal == NULL)
		return NULL;
	ctorRemote = (*env)->GetMethodID(env, linkInfoRemoteClass, "<init>",
			"(FFFFLjava/lang/String;IZ)V");
	if (ctorRemote == NULL)
		return NULL;
	...
	for (link = list; link; link = link->next) {
		...
		switch (link->dest.kind) {
		case FZ_LINK_GOTO: {
			linkInfo = (*env)->NewObject(env, linkInfoInternalClass,
					ctorInternal, (float) rect.x0, (float) rect.y0,
					(float) rect.x1, (float) rect.y1, link->dest.ld.gotor.page);
			break;
		}

		case FZ_LINK_GOTOR: {
			jstring juri = (*env)->NewStringUTF(env,
					link->dest.ld.gotor.file_spec);
			linkInfo = (*env)->NewObject(env, linkInfoRemoteClass, ctorRemote,
					(float) rect.x0, (float) rect.y0, (float) rect.x1,
					(float) rect.y1, juri, link->dest.ld.gotor.page,
					link->dest.ld.gotor.new_window ? JNI_TRUE : JNI_FALSE);
			break;
		}

		case FZ_LINK_URI: {
			jstring juri = (*env)->NewStringUTF(env, link->dest.ld.uri.uri);
			linkInfo = (*env)->NewObject(env, linkInfoExternalClass,
					ctorExternal, (float) rect.x0, (float) rect.y0,
					(float) rect.x1, (float) rect.y1, juri);
			break;
		}
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_getWidgetAreasInternal) (JNIEnv * env, jobject thiz,
		int pageNumber) {
	jclass rectFClass;
	jmethodID ctor;
	jobjectArray arr;
	...
	count = 0;
	for (widget = pdf_first_widget(idoc, (pdf_page *) pc->page); widget;
			widget = pdf_next_widget(widget))
		count++;
	...
	for (widget = pdf_first_widget(idoc, (pdf_page *) pc->page); widget;
		...
		rectF = (*env)->NewObject(env, rectFClass, ctor, (float) rect.x0,
...
JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_getAnnotationsInternal) (JNIEnv * env, jobject thiz,
		int pageNumber) {
	jclass annotClass;
	jmethodID ctor;
	jobjectArray arr;
	...
	count = 0;
	for (annot = fz_first_annot(glo->doc, pc->page); annot; annot =
			fz_next_annot(glo->doc, annot))
		count++;
	...
	for (annot = fz_first_annot(glo->doc, pc->page); annot; annot =
		...
		jannot = (*env)->NewObject(env, annotClass, ctor, (float) rect.x0,
				(float) rect.y0, (float) rect.x1, (float) rect.y1, type);
		if (jannot == NULL)
			return NULL;
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_passClickEventInternal) (JNIEnv * env, jobject thiz,
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_setFocusedWidgetTextInternal) (JNIEnv * env, jobject thiz,
		jstring jtext) {
	const char *text;
	int result = 0;
	globals *glo = get_globals(env, thiz);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_setFocusedWidgetChoiceSelectedInternal) (JNIEnv * env,
		jobject thiz, jobjectArray arr) {
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	pdf_document *idoc = pdf_specifics(glo->doc);
	pdf_widget *focus;
...
JNI_FN(MuPDFCore_waitForAlertInternal) (JNIEnv * env, jobject thiz) {
	...
	ctor = (*env)->GetMethodID(env, alertClass, "<init>",
	...
	return (*env)->NewObject(env, alertClass, ctor, message, alert.icon_type,
			alert.button_group_type, title, alert.button_pressed);
}

JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_replyToAlertInternal) (JNIEnv * env, jobject thiz,
		jobject alert) {
	globals *glo = get_globals(env, thiz);
	jclass alertClass;
	jfieldID field;
