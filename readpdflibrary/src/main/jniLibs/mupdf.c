...
static int bufferStreamRead(fz_stream *stream, unsigned char *buf, int len) {
    ...
    jbyteArray
    array = (jbyteArray)(
    void *)((*env)->GetObjectField(env, glo->thiz, buffer_fid));
...
JNI_FN(MuPDFCore_countPagesInternal)(JNIEnv
*env,
jobject thiz
)
{
...
JNI_FN(MuPDFCore_gotoPageInternal)(JNIEnv
*env,
jobject thiz,
int page
)
...
JNI_FN(MuPDFCore_getPageWidth)(JNIEnv
*env,
jobject thiz
)
{
...
return glo->pages[glo->current].
width;
}

JNIEXPORT float JNICALL
JNI_FN(MuPDFCore_getPageHeight)(JNIEnv
*env,
jobject thiz
)
{
...
JNIEXPORT jbooleanJNICALL
                  JNI_FN(MuPDFCore_drawPage)(JNIEnv *env, jobject thiz, jobject bitmap,
                                             int pageW, int pageH, int patchX, int patchY,
...
JNIEXPORT jbooleanJNICALL
                  JNI_FN(MuPDFCore_updatePageInternal)(JNIEnv *env, jobject thiz, jobject bitmap,
                                                       int page,
                                                       int pageW, int pageH, int patchX, int patchY,
                                                       int patchW, int patchH)
{
    ...
    if (pc == NULL) {
        ...
        return JNI_FN(MuPDFCore_drawPage)(env, thiz, bitmap, pageW, pageH, patchX, patchY, patchW,
                                          patchH);
    }
...
static int
fillInOutlineItems(JNIEnv *env, jclass olClass, jmethodID ctor, jobjectArray arr, int pos,
                   fz_outline *outline, int level) {
    while (outline) {
        if (outline->dest.kind == FZ_LINK_GOTO) {
        ...
        for (b = 0; b < text->len; b++) {
            ...
            for (l = 0; l < block->len; l++) {
                ...
                for (s = 0, span = line->first_span; span; s++, span = span->next) {
                    jobjectArray *carr = (*env)->NewObjectArray(env, span->len, textCharClass,
                                                                NULL);
                    ...
                    for (c = 0; c < span->len; c++) {
                        ...
                        jobject cobj = (*env)->NewObject(env, textCharClass, ctor, bbox.x0, bbox.y0,
                                                         bbox.x1, bbox.y1, ch->c);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addMarkupAnnotationInternal)(JNIEnv
* env,
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal)(JNIEnv
* env,
jobject thiz, jobjectArray
arcs)
{
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_deleteAnnotationInternal)(JNIEnv
* env,
jobject thiz,
int annot_index
)
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_destroying)(JNIEnv
* env,
jobject thiz
)
{
                                                               ...
                                                               int pageNumber)
{
    ...
    count = 0;
    for (widget = pdf_first_widget(idoc, (pdf_page *) pc->page); widget; widget = pdf_next_widget(
            widget))
        count++;
                                                               ...
                                                               int pageNumber)
{
    ...
    count = 0;
    for (annot = fz_first_annot(glo->doc, pc->page); annot; annot = fz_next_annot(glo->doc,
                                                                                  annot)) {
        fz_rect rect;
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_passClickEventInternal)(JNIEnv
* env,
jobject thiz,
int pageNumber,
float x,
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_setFocusedWidgetTextInternal)(JNIEnv
* env,
...
(*env)->
ReleaseStringUTFChars(env, jtext, text
);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_setFocusedWidgetChoiceSelectedInternal)(JNIEnv
* env,
jobject thiz, jobjectArray
arr)
{
...
{
objs[i] = (jstring)(*env)->
GetObjectArrayElement(env, arr, i
);
sel[i] = (char *)(*env)->
GetStringUTFChars(env, objs[i], NULL
);
}
...
i++)
(*env)->
ReleaseStringUTFChars(env, objs[i], sel[i]
);
...
JNIEXPORT int JNICALL
JNI_FN(MuPDFCore_getFocusedWidgetTypeInternal)(JNIEnv
* env,
jobject thiz
)
{
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_replyToAlertInternal)(JNIEnv
* env,
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_startAlertsInternal)(JNIEnv
* env,
jobject thiz
)
{
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_stopAlertsInternal)(JNIEnv
* env,
jobject thiz
)
{
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_saveInternal)(JNIEnv
* env,
jobject thiz
)
{
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_dumpMemoryInternal)(JNIEnv
* env,
jobject thiz
)
{
