...
public class ImageLoaderHelp {
    ...
    public static void initImageLoader(Context context) {
        // 初始化ImageLoader
        File cachefile = new File(IMAGE_LOAD_CACHE_PATH);// 获取到缓存的目录地址
        if (!cachefile.exists()) {
            cachefile.mkdirs();
        }
        ...
        // 设置ImageLoader的配置参
        builder.defaultDisplayImageOptions(getDisplayImageOptions(20));
        // 设置图片请求打印日志
     ...
     * 配置ImageLoder
     */
    public static DisplayImageOptions getDisplayImageOptions(int round_number) {
        ...
        DisplayImageOptions options = new DisplayImageOptions.Builder().imageScaleType(ImageScaleType.EXACTLY)
                ...
                .displayer(new RoundedBitmapDisplayer(round_number)) // 设置成圆角图片
